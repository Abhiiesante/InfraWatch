"""
Bronze Ingestion Pipeline: Inspections
Reads raw inspection JSON from landing zone and losslessly appends to Bronze Delta table.
"""
import os
import sys
import uuid
...
RAW_DATA_PATH = f"{DATA_LAKE_PATH}/workspace/default/infrawatch_raw/inspections/*/*.json"
...
def run():
    print(f"Starting Bronze Inspection Ingestion from {RAW_DATA_PATH}")
    spark = SparkSession.builder.getOrCreate()

    try:
        import glob
        if not glob.glob(RAW_DATA_PATH):
            print(f"No raw inspection files found in {RAW_DATA_PATH}. Skipping Bronze inspection ingestion.")
            return
        ...
        print(f"Successfully ingested inspections to {BRONZE_TABLE_PATH}")
    except Exception as e:
        print(f"Error processing bronze inspections: {e}")
        import traceback
        traceback.print_exc()
