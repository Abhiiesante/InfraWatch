"""
Bronze Ingestion Pipeline: Image Metadata
Reads raw image/frame metadata JSON from landing zone and losslessly appends to Bronze Delta table.
"""
import os
import sys
import uuid
...
RAW_DATA_PATH = f"{DATA_LAKE_PATH}/workspace/default/infrawatch_raw/image_metadata/*/*.json"
...
def run():
    print(f"Starting Bronze Image Metadata Ingestion from {RAW_DATA_PATH}")
    spark = SparkSession.builder.getOrCreate()

    try:
        import glob
        if not glob.glob(RAW_DATA_PATH):
            print(f"No raw image metadata files found in {RAW_DATA_PATH}. Skipping Bronze image metadata ingestion.")
            return
        ...
        print(f"Successfully ingested image metadata to {BRONZE_TABLE_PATH}")
    except Exception as e:
        print(f"Error processing bronze image metadata: {e}")
        import traceback
        traceback.print_exc()
