"""
Bronze Ingestion Pipeline: Incidents
Reads raw incident JSON from landing zone and losslessly appends to Bronze Delta table.
"""
import os
import sys
import uuid
...
RAW_DATA_PATH = f"{DATA_LAKE_PATH}/workspace/default/infrawatch_raw/incidents/*/*.json"
...
def run():
    print(f"Starting Bronze Incident Ingestion from {RAW_DATA_PATH}")
    spark = SparkSession.builder.getOrCreate()

    try:
        import glob
        if not glob.glob(RAW_DATA_PATH):
            print(f"No raw incident files found in {RAW_DATA_PATH}. Skipping Bronze incident ingestion.")
            return
        ...
        print(f"Successfully ingested incidents to {BRONZE_TABLE_PATH}")
    except Exception as e:
        print(f"Error processing bronze incidents: {e}")
        import traceback
        traceback.print_exc()
