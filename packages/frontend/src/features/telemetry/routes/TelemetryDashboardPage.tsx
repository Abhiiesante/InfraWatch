    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800 backdrop-blur-md">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-slate-100">Live IoT Telemetry & Sensor Engine</h1>
          </div>
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="px-4 py-2 bg-slate-800 border border-slate-700 rounded-xl text-slate-200 focus:outline-none focus:border-indigo-500"
          >
            {assets.map((asset) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-card p-6 rounded-2xl border border-white/10 relative overflow-hidden group">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Stream Engine</span>
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-extrabold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 glow-emerald">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" /> Real-time Streaming
            </span>
          </div>
          <div className="text-3xl font-black text-slate-100 mt-3">{readings.length} Packets Received</div>
          <p className="text-xs text-slate-400 mt-1 font-medium">Last tick {readings[0] ? new Date(readings[0].timestamp).toLocaleTimeString() : 'just now'}</p>
        </div>

        <div className="glass-card p-6 rounded-2xl border border-white/10 relative overflow-hidden group">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Active Sensor Metric</span>
          ...
          <p className="text-xs text-indigo-300 mt-1 font-semibold">Type: {readings[0]?.sensorType || 'TEMPERATURE'}</p>
        </div>

        <div className="glass-card p-6 rounded-2xl border border-white/10 relative overflow-hidden group">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Rule Evaluators</span>
            <Sliders className="w-5 h-5 text-purple-400" />
          </div>
          <div className="text-3xl font-black text-purple-300 mt-3">{rules.length} Active Rules</div>
          <p className="text-xs text-slate-400 mt-1 font-medium">Auto-triggers alert notifications on breach</p>
        </div>
      </div>

      {/* Main Streaming Chart */}
      <div className="p-6 bg-slate-900/60 rounded-2xl border border-slate-800 backdrop-blur-md">
        <h3 className="text-base font-semibold text-slate-200 mb-4">Real-Time Sensor Telemetry Stream</h3>
        {isLoading ? (
          <div className="h-64 flex items-center justify-center text-slate-500">Connecting to stream...</div>
        ) : (
          <div className="h-72 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                />
                <Line type="monotone" dataKey="value" stroke="#818cf8" strokeWidth={3} dot={false} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
      ...
      <div className="bg-slate-900/60 rounded-2xl border border-slate-800 overflow-hidden">
        <div className="p-5 border-b border-slate-800 flex items-center justify-between">
          <h3 className="text-base font-semibold text-slate-200">Active Sensor Threshold Rules</h3>
          <span className="text-xs text-slate-400">Automated triggers generate alerts on breach</span>
        </div>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {(Array.isArray(rules) ? rules : []).map((rule) => (
                <tr key={rule.id} className="hover:bg-slate-800/30">
                  <td className="px-6 py-4 font-semibold text-slate-200">{rule.sensorType}</td>
                  <td className="px-6 py-4">{rule.asset?.name || 'All Assets (Global)'}</td>
                  <td className="px-6 py-4 text-slate-400">{rule.minThreshold ?? '—'}</td>
                  <td className="px-6 py-4 text-amber-400 font-semibold">{rule.maxThreshold ?? '—'}</td>
