          ...
          <form onSubmit={handleSubmit} className="space-y-5 py-2">
            <div className="space-y-2">
              ...
              <input
                ...
                className="flex h-11 w-full rounded-xl border border-white/20 glass-panel/50 px-4 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary  hover: transition-all"
            ...
            {triageResult && !isTriaging && (
              <div className="p-4 rounded-xl bg-gradient-to-r from-purple-500/10 to-primary/10 border border-purple-500/30 text-xs text-slate-800 space-y-2 animate-in fade-in">
                <div className="flex items-center justify-between font-bold">
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-purple-600 text-slate-800 font-bold hover:bg-purple-700 transition-colors"
                  >
                    Apply Suggestion
                  </button>
                </div>
                <p className="text-slate-800/80">
                  Category: <strong className="text-[#3A4046]">{triageResult.suggestedCategory}</strong> | Suggested Severity: <strong className="text-slate-800">{triageResult.suggestedSeverity}</strong>
                </p>
                <p className="italic text-[11px] text-slate-800/70">{triageResult.rationale}</p>
              </div>
            )}

            <div className="space-y-2">
              <label htmlFor="inc-asset" className="text-sm font-medium leading-none text-foreground">Associated Asset (Optional)</label>
              <select id="inc-asset" name="assetId" className="flex h-11 w-full rounded-xl border border-white/20 glass-panel/50 px-4 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary  hover: transition-all">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="flex min-h-[90px] w-full rounded-xl border border-white/20 glass-panel/50 px-4 py-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary  hover: transition-all resize-y"
            ...
            <div className="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-3 pt-4">
              ...
              <button
                disabled={isPending}
                type="submit"
                className="inline-flex items-center justify-center rounded-xl text-sm font-semibold transition-all bg-primary text-slate-800 hover:bg-primary/90  hover:shadow-primary/30 h-11 px-8 py-2"
              >
