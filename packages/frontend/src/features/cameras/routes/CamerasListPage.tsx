    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 w-full animate-in fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Dialog.Root open={showCreate} onOpenChange={setShowCreate}>
          <Dialog.Trigger asChild>
            <button className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-6 py-2.5 rounded-xl font-bold hover:shadow-lg hover:shadow-purple-500/30 hover:-translate-y-0.5 transition-all duration-300 flex items-center gap-2">
          ...
          <Dialog.Portal>
            ...
            <Dialog.Content className="fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 p-8 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl">
              <div className="flex flex-col space-y-1.5">
                <Dialog.Title className="text-2xl font-bold text-slate-900 dark:text-white">Register Camera Stream</Dialog.Title>
              ...
              <form onSubmit={handleCreateCamera} className="space-y-4 py-2">
                <div className="space-y-1.5">
                  <label htmlFor="cam-name" className="text-xs font-bold text-slate-700 dark:text-slate-300">Camera Name</label>
                  <input required id="cam-name" name="name" className="flex h-11 w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-4 text-sm" placeholder="e.g. Tokyo Harbour Bridge Live Cam" />
                </div>
                <div className="space-y-1.5">
                  <label htmlFor="cam-asset" className="text-xs font-bold text-slate-700 dark:text-slate-300">Associated Infrastructure Facility</label>
                  <select required id="cam-asset" name="assetId" className="flex h-11 w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-4 text-sm">
                ...
                <div className="space-y-1.5">
                  <label htmlFor="cam-type" className="text-xs font-bold text-slate-700 dark:text-slate-300">Camera Hardware Type</label>
                  <select required id="cam-type" name="cameraType" className="flex h-11 w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-4 text-sm">
                ...
                <div className="space-y-1.5">
                  <label htmlFor="cam-rtsp" className="text-xs font-bold text-slate-700 dark:text-slate-300">Stream URL</label>
                  <input required id="cam-rtsp" name="rtspUrl" className="flex h-11 w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-4 text-sm font-mono" placeholder="https://..." />
                </div>
                <div className="space-y-1.5">
                  <label htmlFor="cam-ip" className="text-xs font-bold text-slate-700 dark:text-slate-300">IP Address</label>
                  <input id="cam-ip" name="ipAddress" className="flex h-11 w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-4 text-sm font-mono" placeholder="192.168.10.100" />
                </div>
                <div className="flex justify-end gap-3 pt-4">
                  <Dialog.Close asChild>
                    <button type="button" className="px-5 py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-100 rounded-xl">Cancel</button>
                  </Dialog.Close>
                  <button disabled={isCreating} type="submit" className="px-6 py-2.5 text-sm font-bold bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-lg shadow-indigo-500/20">
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-24">
          ...
          <p className="text-slate-500 font-medium">Connecting to surveillance streams...</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-16 text-center shadow-sm">
          ...
          <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">No active camera streams</h3>
          <p className="text-slate-500 max-w-md mx-auto text-sm">Add your first surveillance feed to view real-time optical streams.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {data?.cameras?.map((camera: any) => {
            ...
            return (
              <div
                key={camera.id}
                className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group flex flex-col"
              >
                ...
                <div
                  ...
                  className="h-52 bg-slate-950 relative overflow-hidden cursor-pointer group/feed"
                >
                  <HlsVideoPlayer
                    src={hlsUrl}
                    poster={posterImg}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-b from-black/60 via-transparent to-black/70 p-3.5 flex flex-col justify-between pointer-events-none">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 bg-black/60 px-2.5 py-1 rounded-full border border-emerald-500/40 backdrop-blur-md">
                        ...
                        <span className="text-[10px] font-mono font-bold text-white">24/7 OPEN WEBCAM</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 backdrop-blur-md flex items-center gap-1">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/feed:opacity-100 transition-opacity flex items-center justify-center">
                    <div className="w-12 h-12 rounded-full bg-indigo-600 text-white flex items-center justify-center shadow-xl transform scale-90 group-hover/feed:scale-100 transition-transform">
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    <span className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2.5 py-1 rounded-md font-bold">
                      {camera.cameraType}
                    </span>
                    {camera.ipAddress && (
                      <span className="bg-slate-100 dark:bg-slate-800 text-slate-500 px-2 py-1 rounded-md font-mono">
                        {camera.ipAddress}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <button
                      ...
                      className="px-4 py-2 rounded-xl text-xs font-bold bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-600 hover:text-white transition-all flex items-center gap-1.5 border border-indigo-200 dark:border-indigo-800"
                    >
          ...
          <div className="bg-slate-900 border border-slate-800 w-full max-w-5xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            ...
            >
              ...
              {aiOverlay && isPlaying && (
                ...
                >
                  <span className="bg-rose-600 text-white font-extrabold text-xs px-3 py-1 rounded-md shadow-lg flex items-center gap-2">
            ...
            <div className="p-4 bg-slate-950 border-t border-slate-800 flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${
                    isPlaying ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-emerald-600 text-white shadow'
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${
                    useWebcam ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/40 ring-2 ring-emerald-400 animate-pulse' : 'bg-slate-800 text-slate-300 hover:text-white'
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${
                    thermalFilter ? 'bg-orange-600 text-white shadow-lg shadow-orange-600/40 ring-2 ring-orange-400' : 'bg-slate-800 text-slate-300 hover:text-white'
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${
                    aiOverlay ? 'bg-purple-600 text-white shadow-lg shadow-purple-600/40 ring-2 ring-purple-400' : 'bg-slate-800 text-slate-300 hover:text-white'
                  }`}
                >
              ...
              <div className="flex items-center gap-3">
                <span className="text-xs font-bold text-slate-400 flex items-center gap-1">
                  <Move className="w-3.5 h-3.5 text-indigo-400" /> PTZ Optics:
                </span>
                <div className="flex items-center gap-1 bg-slate-900 p-1 rounded-xl border border-slate-800">
                  <button
                    onClick={() => setPanX(x => Math.max(-30, x - 10))}
                    className="px-3 py-1 text-xs font-bold text-slate-200 hover:bg-indigo-600 hover:text-white rounded transition-colors"
                    title="Pan Left"
                  >◀</button>
                  <button
                ...
                <button
                  ...
                  className="p-2 bg-slate-800 hover:bg-indigo-600 text-slate-200 hover:text-white rounded-xl transition-colors"
                  title="Zoom In (+)"
                >
                ...
                {(zoom !== 1.0 || panX !== 0 || panY !== 0) && (
                  <button
                    onClick={resetPTZ}
                    className="p-2 bg-rose-900/60 hover:bg-rose-600 text-white rounded-xl transition-colors"
