...
export const AnomalyDetailPage = () => {
  ...
  if (!anomaly) {
    return <div className="p-8 text-center text-slate-500">Anomaly flag not found</div>;
  }
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8 w-full animate-in fade-in">
      <Link to="/anomalies" className="inline-flex items-center text-sm font-bold text-slate-500 hover:text-purple-600 transition-colors bg-white/50 dark:bg-slate-800/50 px-4 py-2 rounded-lg border border-slate-200 shadow-sm backdrop-blur-sm w-fit">
        ...
        <div className="flex flex-col lg:flex-row items-start justify-between gap-6 relative z-10">
          <div>
            <div className="flex items-center gap-3">
              <span className="px-3 py-1 rounded-full text-xs font-extrabold bg-purple-100 text-purple-800 dark:bg-purple-900/40 dark:text-purple-300 border border-purple-300">
            ...
            <p className="text-slate-500 mt-1 font-medium">
              Camera: <strong className="text-slate-700 dark:text-slate-200">{anomaly.camera?.name}</strong> | Asset: <strong className="text-slate-700 dark:text-slate-200">{anomaly.camera?.asset?.name}</strong>
            </p>
          </div>
            ...
            {anomaly.status === 'PENDING_REVIEW' ? (
              <>
                <button
                  ...
                  className="px-6 py-3 rounded-xl font-bold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-200 transition-all flex items-center gap-2 border border-slate-300"
                >
                ...
                <button
                  ...
                  className="px-6 py-3 rounded-xl font-bold bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-lg shadow-purple-500/30 hover:shadow-purple-500/50 transition-all flex items-center gap-2"
                >
            ...
            <div className="flex items-center gap-1 bg-slate-900 p-1 rounded-xl border border-slate-700">
              <button
                ...
                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${
                  streamMode === 'SNAPSHOT' ? 'bg-purple-600 text-white shadow' : 'text-slate-400 hover:text-white'
                }`}
              >
                Snapshot Frame
              </button>
              <button
                ...
                className={`px-3 py-1 text-xs font-bold rounded-lg transition-all flex items-center gap-1.5 ${
                  streamMode === 'LIVE' ? 'bg-emerald-600 text-white shadow' : 'text-slate-400 hover:text-white'
                }`}
              >
          ...
          <div className="relative rounded-xl overflow-hidden border border-purple-500/30 shadow-2xl bg-black aspect-video flex items-center justify-center">
              ...
              <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-4 bg-gradient-to-b from-black/50 via-transparent to-black/60">
                ...
                <div className="flex items-center justify-between text-xs font-mono font-bold text-emerald-400">
                  <div className="flex items-center gap-2 bg-black/60 px-3 py-1 rounded-full border border-emerald-500/40 backdrop-blur-md">
                    ...
                    <span className="text-white">REC ⏺ 30.0 FPS</span>
                  </div>
                ...
                <div className="absolute inset-x-14 inset-y-10 border-2 border-rose-500 bg-rose-500/10 rounded-lg animate-pulse pointer-events-none flex items-start justify-start p-2">
                  <span className="bg-rose-600 text-white font-extrabold text-xs px-2.5 py-1 rounded shadow-lg flex items-center gap-1.5">
            ...
            {streamMode === 'SNAPSHOT' && (
              <div className="absolute inset-x-12 inset-y-12 border-4 border-rose-500 bg-rose-500/10 rounded-lg animate-pulse pointer-events-none flex items-start justify-start p-2">
                <span className="bg-rose-600 text-white font-extrabold text-xs px-2 py-1 rounded shadow-md">
        ...
        <div className="space-y-6">
          <div className="glass rounded-2xl border border-white/20 p-6 shadow-xl space-y-4">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between py-2 border-b border-white/10">
                ...
                <span className="font-bold text-slate-800 dark:text-white">YOLOv8 Industrial CV</span>
              </div>
              <div className="flex justify-between py-2 border-b border-white/10">
                <span className="text-slate-500 font-medium">Confidence Score:</span>
                <span className="font-extrabold text-purple-600 dark:text-purple-400">{confDisplay}%</span>
              </div>
              <div className="flex justify-between py-2 border-b border-white/10">
                <span className="text-slate-500 font-medium">Flag Type:</span>
                <span className="font-bold text-slate-800 dark:text-white">{primaryDetection.label}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-white/10">
                <span className="text-slate-500 font-medium">Tenant Threshold:</span>
                <span className="font-bold text-slate-800 dark:text-white">75.0%</span>
              </div>
              <div className="flex justify-between py-2 border-b border-white/10">
                <span className="text-slate-500 font-medium">Stream Status:</span>
                <span className="font-extrabold text-emerald-600 flex items-center gap-1">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping" /> Live RTSP Active
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="glass rounded-2xl border border-white/20 p-6 shadow-xl">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-bold text-slate-900 dark:text-white text-sm">Human-in-the-Loop Safeguard</h4>
