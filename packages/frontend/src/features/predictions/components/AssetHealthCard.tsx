...
export const AssetHealthCard = ({ score, assetName, assetType }: AssetHealthCardProps) => {
  const getScoreColor = (val: number) => {
    if (val >= 80) return { stroke: '#10b981', text: 'text-emerald-500', bg: 'bg-emerald-500/10 border-emerald-500/30' };
    if (val >= 55) return { stroke: '#f59e0b', text: 'text-amber-500', bg: 'bg-amber-500/10 border-amber-500/30' };
    return { stroke: '#f43f5e', text: 'text-rose-500', bg: 'bg-rose-500/10 border-rose-500/30' };
  };
  ...
  return (
    <div className={`p-5 rounded-2xl border backdrop-blur-md transition-all duration-300 hover:shadow-lg ${style.bg} flex items-center justify-between`}>
      <div className="space-y-1">
        ...
        <h4 className="font-extrabold text-lg text-slate-900 dark:text-white">{assetName}</h4>
      ...
      <div className="relative w-20 h-20 flex items-center justify-center flex-shrink-0">
        <svg className="w-full h-full transform -rotate-90" viewBox="0 0 90 90">
          <circle cx="45" cy="45" r={radius} stroke="currentColor" strokeWidth="7" fill="transparent" className="text-slate-200 dark:text-slate-700" />
          <circle
            cx="45"
            cy="45"
            r={radius}
