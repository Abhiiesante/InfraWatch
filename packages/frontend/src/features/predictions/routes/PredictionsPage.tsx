  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 w-full animate-in fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 dark:from-emerald-300 dark:to-cyan-300 drop-shadow-sm flex items-center gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass rounded-2xl p-6 border border-white/20 shadow-xl flex items-center justify-between">
          ...
          <div className="w-16 h-16 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-500 font-black text-2xl">
        ...
        <div className="glass rounded-2xl p-6 border border-white/20 shadow-xl flex items-center justify-between">
          ...
          <div className="w-16 h-16 rounded-2xl bg-rose-500/10 border border-rose-500/30 flex items-center justify-center text-rose-500 font-bold">
          ...
          {assetsData?.assets?.map((asset: any) => (
            <div key={asset.id} className="relative group">
              ...
              <button
                ...
                className="mt-2 w-full py-2 bg-white/40 dark:bg-slate-800/40 hover:bg-emerald-500 text-slate-700 hover:text-white dark:text-slate-300 font-bold text-xs rounded-xl border border-white/20 transition-all duration-300 flex items-center justify-center gap-2 shadow-sm"
              >
                {isRunning && selectedAssetId === asset.id ? (
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
                ) : (
                  <Play className="w-3.5 h-3.5 text-emerald-500 group-hover:text-white" />
                )}
      ...
      <div className="glass rounded-2xl border border-white/20 overflow-hidden shadow-xl space-y-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-100/50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-300 font-bold uppercase tracking-wider text-xs">
              <tr>
              ...
              {isPredLoading ? (
                <tr>
                  <td colSpan={5} className="px-8 py-16 text-center">
                    ...
                    <p className="text-slate-500 font-medium">Computing time-series probabilities...</p>
                  </td>
                </tr>
                ...
                predictionsData?.predictions?.map((pred: any) => (
                  <tr key={pred.id} className="hover:bg-white/60 dark:hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-8 py-4 text-xs font-medium text-slate-700 dark:text-slate-300 max-w-xs truncate">
                      {pred.recommendedAction}
                    </td>
                    <td className="px-8 py-4">
                      {pred.autoScheduledInspectionId ? (
                        <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-200">
